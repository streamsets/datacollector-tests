...
@sdc_min_version('2.8.0.0')
def test_schema_generator_processor(sdc_builder, sdc_executor):
    """
    Schema generated by the schema generator processor is well exercised by unit test, so this integration
    test more focuses on interoperability with our Avro DataFormat library. We create rather complex record
    with all different data types, generate schema for it, write it to Avro file using the generated schema
    and in second pipeline read it back. We assert that we read back (using the generated schema) is what we had
    at the beginning.
    """
    ...
    dev_raw_data_source >> field_type_converter >> schema_generator >> local_fs
    ...
    # Execute first pipeline, it will stop after processing the first batch

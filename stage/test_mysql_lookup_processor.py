...
def test_mysql_lookup_processor(sdc_builder, sdc_executor, database, credential_store):
    ...
    The pipeline looks like:
        dev_raw_data_source >> mysql_lookup >> wiretap
    """
    ...
    mysql_lookup = pipeline_builder.add_stage('MySQL Lookup')
    query_str = f'SELECT `name` FROM `{table_name}` WHERE `id` = ${{record:value("/id")}}'
    column_mappings = [dict(dataType='USE_COLUMN_TYPE',
                            columnName='name',
                            field='/FirstName')]
    mysql_lookup.set_attributes(sql_query=query_str,
                               column_mappings=column_mappings)

    wiretap = pipeline_builder.add_wiretap()
    dev_raw_data_source >> mysql_lookup >> wiretap.destination
    pipeline = pipeline_builder.build(title='MySQL Lookup').configure_for_environment(database, credential_store)

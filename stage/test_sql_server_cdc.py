...
def test_sql_server_cdc_with_zero_initial_offset(sdc_builder, sdc_executor, database):
    """Test for SQL Server CDC origin stage with "0x0" initial offset
...
def test_sql_server_cdc_on_unknown_type_action(sdc_builder, sdc_executor, database):
    """Set the 'On Unknown Type' action to CONVERT_TO_STRING in SQL CDC origin
    ...
    The template for this test is copied from test_sql_server_cdc_source_table_in_record_header()
    The pipeline will look like:
        sqlserver_cdc_origin >> trash
    """
    connection = None
    if not database.is_cdc_enabled:
        pytest.skip('Test only runs against SQL Server with CDC enabled.')
    ...
    try:
        ...
        logger.info('Creating table %s, in schema %s...', table_name, schema_name)
        ...
        # Setup CDC on table
        # exec sys.sp_cdc_enable_table @source_schema=schema_name,@source_name=table_name, @capture_instance=instance_name, @supports_net_changes=1, @role_name=NULL
        connection.execute(f'exec sys.sp_cdc_enable_table @source_schema=\'{schema_name}\', '
                        f'@source_name=\'{table_name}\', '
                        f'@capture_instance=\'{capture_instance_name}\', '
                        f'@supports_net_changes=1, @role_name=NULL')
        ...
        logger.info('Adding DATETIMEOFFSET record into table %s.%s...', schema_name, table_name)
         # Add DATETIMEOFFSET record to table
        connection.execute(f"INSERT INTO {schema_name}.{table_name} VALUES (1, {INPUT_DATE})")
        ...
        # assert all the data captured have the same raw_data
        output_records = snapshot.snapshot_batches[0][sql_server_cdc.instance_name].output
        assert len(output_records) == 1
        assert output_records[0].field == EXPECTED_OUTCOME

    finally:
        logger.info('Dropping table %s.%s in %s database...', schema_name, table_name, database.type)

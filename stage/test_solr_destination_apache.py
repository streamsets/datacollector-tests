...
def test_solr_write_records_apache(sdc_builder, sdc_executor, solr):
    ...
    doc_title = get_random_string(string.ascii_letters, 10)
    solr_field_name = get_random_string(string.ascii_letters, 10)
    json_fields_map = [{'field': '/id', 'solrFieldName': 'id'},
                       {'field': '/title', 'solrFieldName': solr_field_name}]
    json_str = json.dumps(dict(id=doc_id, title=doc_title))
    ...
    try:
        ...
        query = f'{{!term f=id}}{doc_id}'
        results = client.search(q=query)
        assert len(results) > 0
        assert results.docs[0][solr_field_name][0] == doc_title
    finally:
        # Cleanup the document created by the test.
        client.delete(id=doc_id)
        # Cleanup the field created by the test.
        _delete_schema_field(client, solr_field_name)
...
@solr
@sdc_min_version('3.9.0')
def test_solr_manual_mapping_validation_error(sdc_builder, sdc_executor, solr):
    """Test Solr stage validation when using a user-defined list of record-to-Solr field name mappings.

    We define an incomplete mappings list, missing a required and an optional Solr fields. The expected behavior is to
    signal 2 validation errors: a SOLR_12 error for the missing required field and a SOLR_13 for the required one.
    ...
    # Dev raw data source.
    raw_data = [{'id': '123', 'title': 'Custom title'}]
    dev_raw_data_source = builder.add_stage('Dev Raw Data Source')
    dev_raw_data_source.set_attributes(data_format='JSON',
                                       raw_data=''.join(json.dumps(d) for d in raw_data))

    # Omit 'id' (required field) and '_text_' (optional field).
    field_mapping = [{'field': '/title', 'solrFieldName': 'title'},
                     {'field': '/_root_', 'solrFieldName': '_root_'},
                     {'field': '/_version_', 'solrFieldName': '_version_'}]
    ...
    # Check that there are only 2 issues and they are related to Solr stage.
    assert err.value.issues['issueCount'] == 2
    assert solr_target.instance_name in err.value.issues['stageIssues']
    assert len(err.value.issues['stageIssues'][solr_target.instance_name]) == 2

    # Check that the issues are the expected ones.
    issues = sorted(err.value.issues['stageIssues'][solr_target.instance_name], key=lambda e: e['message'])
    assert issues[0]['message'].startswith('SOLR_12')
    assert 'id' in issues[0]['message'].split(':')[1]  # 'id' is in the list of missing required fields.
    assert issues[1]['message'].startswith('SOLR_13')
    assert '_text_' in issues[1]['message'].split(':')[1]  # '_text_' is in the list of missing optional fields.

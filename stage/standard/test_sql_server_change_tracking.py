...
# CT currently doesn't support special characters in table names
...
def test_dataflow_events(sdc_builder, sdc_executor, database, keep_data):
    ...
    builder = sdc_builder.get_pipeline_builder()
    source = builder.add_stage('SQL Server Change Tracking Client')
    source.fetch_size = 1
    source.table_configs = [
        {'initialOffset': 0, 'schema': 'dbo', 'tablePattern': f'{table_a}'},
        {'initialOffset': 0, 'schema': 'dbo', 'tablePattern': f'{table_b}'}
    ]
    ...
    try:
        ...
        connection.execute(f'ALTER TABLE [{table_a}] ENABLE change_tracking WITH (track_columns_updated = on)')
        b.create(connection.engine)
        connection.execute(f'ALTER TABLE [{table_b}] ENABLE change_tracking WITH (track_columns_updated = on)')
        ...
        assert records[0].header.values['sdc.event.type'] == 'table-finished'
        assert records[0].field['table'] == table_b

        assert records[1].header.values['sdc.event.type'] == 'table-finished'
        assert records[1].field['table'] == table_a

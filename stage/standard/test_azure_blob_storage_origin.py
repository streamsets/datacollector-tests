...
def test_object_names(sdc_builder, sdc_executor, azure, test_name, object_name):
    """Test file names expressions for Blob Storage origin.
    ...
    if Version(sdc_builder.version) >= Version('5.7.0'):
        azure_blob_storage_origin.file_processing_delay_in_ms = 1000
...
def test_data_formats(sdc_builder, sdc_executor):
    pytest.skip("Blob Storage uses DataParserService to parse DataFormats, we do not need to test every format here")
    ...
    try:
        ...
        for starting_point in initial_path:
            for path, data in subdirs_data.items():
                if path != '.':
                    fs.mkdir(os.path.join(rootdir, starting_point, path))
                filepath = os.path.join(rootdir, starting_point, path, 'file.txt')
                fs.touch(filepath)
...
@pytest.mark.parametrize('ignore_empty_blobs', [True, False])
def test_empty_objects(sdc_builder, sdc_executor, azure, ignore_empty_blobs):
    """
    Test that no records are generated if the origin is empty.
    Note that this stage has a checkbox to choose whether to ignore or not empty objects, for example
    when working with WHOLE_FILE, because folders and objects are undistinguished in blob storage

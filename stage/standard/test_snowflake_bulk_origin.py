...
DATA_TYPES_SNOWFLAKE = [
    ...
    ('TIMESTAMP_LTZ',     '2020-02-03 11:12:13',        'ZONED_DATETIME', '2020-02-03T11:12:13-08:00',                '2020-02-03 11:12:13.000 -0800'),
...
@pytest.mark.parametrize('read_values_as_string', [True, False])
def test_long_variant_values(sdc_builder, sdc_executor, snowflake, read_values_as_string):
    """
    Tests that the Snowflake Bulk Origin correctly long variant values over 4096 chars.
    ...
    try:
        ...
        inserted_records = [
            '{' +
                ...
                '"Number":123,' +
                '"Object":{"a":"b","c":"d"},' +
                f'"String":"{get_random_string(string.ascii_uppercase, 10000)}"' +
            '}',
            '{'
                '"Array":[],' +
                '"Boolean":false,' +
        ...
        # Check that the number of records is equal to what we expect
        assert len(records) == len(inserted_records), \
            f'{len(inserted_records)} records should have been processed but only {len(records)} were found'

        for record, expected_record in zip(records, inserted_records):
            assert record.header.values[DATABASE_RECORD_HEADER_ATTRIBUTE_NAME] == DEFAULT_DATABASE
            assert record.header.values[SCHEMA_RECORD_HEADER_ATTRIBUTE_NAME] == DEFAULT_SCHEMA
            assert record.header.values[TABLE_RECORD_HEADER_ATTRIBUTE_NAME] == table_name
            ...
            for i in range(0, len(column_names)):
                ...
                # ... and that the value contained is what we expect
                if read_values_as_string:
                    assert expected_record == record.field[column_names[i]], \
                        f'The value of the field {column_names[i]} should have been {expected_record},' \
                        f' but it is {record.field[column_names[i]]}'
                else:
                    assert json.loads(expected_record) == record.field[column_names[i]], \
                        f'The value of the field {column_names[i]} should have been {expected_record},' \
                        f' but it is {record.field[column_names[i]]}'
    finally:
        snowflake.delete_staged_files(storage_path)

...
def _get_random_name(database, prefix='', length=5):
    ...
    return '{}{}'.format(prefix.lower(), get_random_string(string.ascii_lowercase))
...
def _create_mysql_producer_pipeline(pipeline_builder, pipeline_title, raw_data, table_name, operation):
    """Helper function to create and return a pipeline with MySQL Producer
    The Deduplicator assures there is only one ingest to database. The pipeline looks like:
    The pipeline looks like:
        dev_raw_data_source >> record_deduplicator >> mysql_producer
    ...
    mysql_producer = pipeline_builder.add_stage('MySQL Producer')
    mysql_producer.set_attributes(default_operation=operation,
...
def test_mysql_producer_insert_type_err(sdc_builder, sdc_executor, database):
    ...
    mysql_producer = pipeline_builder.add_stage('MySQL Producer')
    mysql_producer.set_attributes(default_operation='INSERT',
                                 table_name=table_name,
                                 field_to_column_mapping=FIELD_MAPPINGS,
                                 stage_on_record_error='TO_ERROR')
    wiretap = pipeline_builder.add_wiretap()

    dev_raw_data_source >> [mysql_producer, wiretap.destination]

    pipeline = pipeline_builder.build(title="MySQL producer with error")
...
def test_mysql_producer_coerced_insert(sdc_builder, sdc_executor, database):
    ...
    pipeline = _create_mysql_producer_pipeline(pipeline_builder, 'MySQL Producer Insert', DATA, table_name, 'INSERT')
    sdc_executor.add_pipeline(pipeline.configure_for_environment(database))
...
def test_mysql_producer_delete(sdc_builder, sdc_executor, database):
    ...
    pipeline = _create_mysql_producer_pipeline(pipeline_builder, 'MySQL Producer Delete', DATA, table_name, 'DELETE')
    sdc_executor.add_pipeline(pipeline.configure_for_environment(database))
...
def test_mysql_producer_update(sdc_builder, sdc_executor, database):
    ...
    pipeline = _create_mysql_producer_pipeline(pipeline_builder, 'MySQL Producer Update', DATA, table_name, 'UPDATE')
    sdc_executor.add_pipeline(pipeline.configure_for_environment(database))
...
def test_mysql_producer_multirow_with_duplicates(sdc_builder, sdc_executor, database):
    ...
    producer = builder.add_stage('MySQL Producer')
...
def test_mysql_producer_multitable(sdc_builder, sdc_executor, database):
    ...
    pipeline_builder = sdc_builder.get_pipeline_builder()
    pipeline = _create_mysql_producer_pipeline(pipeline_builder, 'MySQL Producer Multitable Insert', INPUT_DATA,
...
def test_error_handling_when_there_is_no_primary_key(sdc_builder, sdc_executor, database, dyn_table, multi_row):
    """
    SDC-12960. Updating MySQL table with no PK results in NPE
    ...
    The pipeline:
    Dev Raw Data Source->Expression Evaluator->Field Remover->MySQL Producer
...
def test_mysql_producer_primary_key_header_update(sdc_builder, sdc_executor, database, multi_row):
    ...
    The pipeline looks like:
        ...
        dev_raw_data_source >> expression_evaluator >> field_remover >> mysql_producer

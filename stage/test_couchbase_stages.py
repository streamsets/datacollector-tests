...
def test_couchbase_destination(sdc_builder, sdc_executor, couchbase):
    ...
    couchbase_destination.set_attributes(database_version='VERSION{}'.format(couchbase.version.split('.')[0]),
                                         unique_document_key_field=document_key_field, bucket=bucket_name)

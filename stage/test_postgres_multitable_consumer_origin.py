...
from stage.utils.utils_primary_key_metadata import PRIMARY_KEY_NON_NUMERIC_METADATA_POSTGRESQL, \
    PRIMARY_KEY_NUMERIC_METADATA_POSTGRESQL, PRIMARY_KEY_POSTGRESQL_TABLE, \
...
def test_postgres_multitable_consumer_with_no_more_data_event_generation_delay(sdc_builder, sdc_executor, database):
    ...
    postgres_multitable_consumer = pipeline_builder.add_stage('PostgreSQL Multitable Consumer')
    postgres_multitable_consumer.no_more_data_event_generation_delay_in_seconds = 1
    postgres_multitable_consumer.table_configs = [{"tablePattern": f'%{src_table}%'}]
    postgres_multitable_consumer.ssl_mode = 'DISABLED'
    ...
    postgres_multitable_consumer >> trash
    postgres_multitable_consumer >= delay
    delay >> trash_event
...
def test_postgres_multitable_consumer_initial_offset_at_the_end(sdc_builder, sdc_executor, database):
    ...
    postgres_multitable_consumer = builder.add_stage('PostgreSQL Multitable Consumer')
    postgres_multitable_consumer.table_configs = [{
...
def test_postgres_multitable_consumer_partitioned_large_offset_gaps(sdc_builder, sdc_executor, database):
    ...
    postgres_multitable_consumer = pipeline_builder.add_stage('PostgreSQL Multitable Consumer')
    postgres_multitable_consumer.set_attributes(table_configs=[{
    ...
    postgres_multitable_consumer >> wiretap.destination

    finisher = pipeline_builder.add_stage("Pipeline Finisher Executor")
    finisher.stage_record_preconditions = ['${record:eventType() == "no-more-data"}']
    postgres_multitable_consumer >= finisher
...
def test_postgres_multitable_consumer_duplicates_read_when_initial_offset_configured(sdc_builder, sdc_executor, database):
    ...
    Pipeline:
        PostgreSQL MT Consumer >> Wiretap
    ...
    postgres_multitable_consumer.number_of_threads = 2
    postgres_multitable_consumer.maximum_pool_size = 2
...
def test_postgres_multitable_consumer_batch_strategy(sdc_builder, sdc_executor, database, batch_strategy, no_of_threads):
    """
    Check if PostgreSQL Multi-table Origin can load a couple of tables without duplicates using both batch_strategy options.
    Also it includes different thread combinations to ensure that with less, same and more threads works fine.

    postgres_multitable_consumer >> wiretap.destination
    postgres_multitable_consumer >= pipeline_finished_executor
    """
    ...
    postgres_multitable_consumer = pipeline_builder.add_stage('PostgreSQL Multitable Consumer')
    postgres_multitable_consumer.set_attributes(table_configs=[{"tablePattern": f'{src_table_prefix}%'}],
...
def test_no_data_losses_or_duplicates_in_multithreaded_mode(sdc_builder, sdc_executor, database, test_data):
    ...
    PostgreSQL Multitable Consumer >> Splitter >> Wiretap1 (12000 records)
    ...
    try:
        ...
        postgres_multitable_consumer = pipeline_builder.add_stage('PostgreSQL Multitable Consumer')
        postgres_multitable_consumer.set_attributes(**attributes)

        stream_selector = pipeline_builder.add_stage('Stream Selector')

        postgres_multitable_consumer >> stream_selector

        event_wiretap = pipeline_builder.add_wiretap()
        postgres_multitable_consumer >= event_wiretap.destination
        ...
        pipeline = pipeline_builder.build().configure_for_environment(database)
        postgres_multitable_consumer.table_configs[0]["tablePattern"] = f'{table_prefix}%'
...
def test_postgres_primary_keys_headers(sdc_builder, sdc_executor, database):
    ...
    origin = pipeline_builder.add_stage('PostgreSQL Multitable Consumer')

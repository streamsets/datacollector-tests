...
def test_blob_storage_origin_with_parquet(sdc_builder, sdc_executor, azure):
...
def test_blob_storage_origin_offset_continuation_token(sdc_builder, sdc_executor, azure):
    """ Test for Blob Storage origin stage. We do so by creating a file in Azure Data Lake Storage using the
    ...
    try:
        ...
        azure_blob_storage_origin >> wiretap.destination
        azure_blob_storage_origin >= finisher

        blob_storage_origin_pipeline = builder.build().configure_for_environment(azure)
        sdc_executor.add_pipeline(blob_storage_origin_pipeline)

        sdc_executor.start_pipeline(blob_storage_origin_pipeline)
        sdc_executor.wait_for_pipeline_metric(blob_storage_origin_pipeline, 'input_record_count', 10, timeout_sec=120)
        sdc_executor.stop_pipeline(blob_storage_origin_pipeline)
...
def test_blob_storage_origin_path_pattern(sdc_builder, sdc_executor, azure, path_pattern):
...
def test_multithreading_multiple_batches(
    ...
    azure_blob_storage_origin = builder.add_stage(name=STAGE_NAME)
    azure_blob_storage_origin.set_attributes(data_format='JSON',
                                             common_path=f'/{directory_name}',
                                             path_pattern='sdc**',
    ...
    azure_blob_storage_origin >> read_wiretap.destination
...
def test_data_format_whole_file_with_post_processing_delete(sdc_builder, sdc_executor, azure):
    """
    This test tries to answer a problem that's coming from an intervention.The test uses a BlobStorageDSource
    ...
    The pipeline looks like:
        azure_blob_storage_origin >> local_fs
    """
    ...
    azure_blob_storage_origin >> local_fs >= wiretap.destination
...
def test_blob_storage_origin_timestamp_sort_with_many_files(sdc_builder, sdc_executor, azure):
    """ Test for Blob Storage origin stage with a large quantity of files and sorting by Timestamp.
    ...
    then reading the files using the Blob Storage Origin Stage, sorting by Timestamp and with a very small value for
    the Batch Wait Time config.
    """
    directory_name = get_random_string(string.ascii_letters, 10)
    ...
    try:
        ...
        wiretap = builder.add_wiretap()
        azure_blob_storage_origin = builder.add_stage(name=STAGE_NAME)
        azure_blob_storage_origin.set_attributes(batch_wait_time_in_ms=400,

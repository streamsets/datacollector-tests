...
def test_snowflake_executor_with_google_cloud_storage_destination(sdc_builder, sdc_executor, snowflake):
    ...
    folder_path = get_random_string(string.ascii_letters, 10)
    storage_path = f'{STORAGE_BUCKET_CONTAINER}/{folder_path}'
    snowflake.create_stage(stage_name, storage_path, stage_location='GCS')
    ...
    gcs_destination = pipeline_builder.add_stage('Google Cloud Storage', type='destination')
    snowflake_gcs_config = snowflake._SnowflakeDataWarehouseWithStages__stages["GCS"]
    gcs_destination.set_attributes(
        bucket=f'{snowflake_gcs_config.gcp_bucket_name}',
        credentials_provider=f'{snowflake_gcs_config.gcp_credentials_provider}',
        common_prefix=f'{folder_path}/',
        data_format='WHOLE_FILE',
        file_name_expression="${record:value('/fileInfo/filename')}"
    )
    ...
    origin >> gcs_destination
    origin >= pipeline_finished
    gcs_destination >= snowflake_executor
    snowflake_executor >= wiretap.destination

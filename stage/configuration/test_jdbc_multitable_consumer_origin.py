...
@database
@pytest.mark.parametrize('connection_string', ['VALID', 'INVALID'])
def test_jdbc_multitable_consumer_origin_configuration_jdbc_connection_string(sdc_builder, sdc_executor, database,
                                                                              connection_string):
    """When connection string is valid then pipeline is able to make database connection and run successfully.
    When we provide invalid connection string then pipeline raises an exception and exits.
    """
    ...
    rows_in_table = [{'id': i, 'name': str(i)} for i in range(1, 5)]
    try:

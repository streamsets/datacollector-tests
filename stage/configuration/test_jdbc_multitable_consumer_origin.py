...
@pytest.mark.parametrize('configuration_value', [False, True])
@pytest.mark.skip('Not yet implemented')
def test_jdbc_multitable_consumer_origin_configuration_auto_commit(sdc_builder, sdc_executor, configuration_value):
    pass
...
@pytest.mark.parametrize('configuration_value', ['ALPHABETICAL', 'NONE', 'REFERENTIAL_CONSTRAINTS'])
@pytest.mark.skip('Not yet implemented')
def test_jdbc_multitable_consumer_origin_configuration_initial_table_order_strategy(sdc_builder, sdc_executor, configuration_value):
    pass
...
@pytest.mark.parametrize('configuration_value', ['DISCARD', 'STOP_PIPELINE', 'TO_ERROR'])
@pytest.mark.skip('Not yet implemented')
def test_jdbc_multitable_consumer_origin_configuration_on_record_error(sdc_builder, sdc_executor, configuration_value):
    pass


@pytest.mark.parametrize('configuration_value', ['CONVERT_TO_STRING', 'STOP_PIPELINE'])
@pytest.mark.skip('Not yet implemented')
def test_jdbc_multitable_consumer_origin_configuration_on_unknown_type(sdc_builder, sdc_executor, configuration_value):
    pass
...
@pytest.mark.parametrize('configuration_value', ['PROCESS_ALL_AVAILABLE_ROWS_FROM_TABLE', 'SWITCH_TABLES'])
@pytest.mark.skip('Not yet implemented')
def test_jdbc_multitable_consumer_origin_configuration_per_batch_strategy(sdc_builder, sdc_executor, configuration_value):
    pass


@database
@pytest.mark.parametrize('queries_per_second', ['0', '2', '5', '10'])
def test_jdbc_multitable_consumer_origin_configuration_queries_per_second(sdc_builder, sdc_executor, database,
                                                                          queries_per_second):
    """SDC will run maximum queries_per_second queries in a second across all partitions and tables.
    If queries_per_second is zero there will be no limit for queries to be run.
    """
    src_table_prefix = get_random_string(string.ascii_lowercase, 6)
    tables = []
    number_of_rows_in_table = 1
    number_of_tables = 20
    try:
        # Created 20 tables
        id_count = 1
        ...
        records_received_count = history.latest.metrics.counter('stage.Trash_01.inputRecords.counter').count

        """Currently using record count in trash stage since we are not able to compare each record values using 
        snapshot since some batches were processed even before snapshot is taken. By giving one second sleep only ensure
        that pipeline ran atleast one second but in real scenarios it run more than one second.So in the assert 
        statement we are ensuring that records count is greater than multiplication of queries_per_second and 
        number_of_rows_in_table.
        """
        if queries_per_second == '0':
            assert records_received_count >= int(queries_per_second)
        else:
            assert records_received_count >= int(queries_per_second * number_of_rows_in_table)
    finally:
        for table in tables:
...
@pytest.mark.parametrize('configuration_value', ['BACKTICK', 'DOUBLE_QUOTES', 'NONE'])
@pytest.mark.skip('Not yet implemented')
def test_jdbc_multitable_consumer_origin_configuration_quote_character(sdc_builder, sdc_executor, configuration_value):
    pass
...
@pytest.mark.parametrize('configuration_value', ['DEFAULT', 'TRANSACTION_READ_COMMITTED', 'TRANSACTION_READ_UNCOMMITTED', 'TRANSACTION_REPEATABLE_READ', 'TRANSACTION_SERIALIZABLE'])
@pytest.mark.skip('Not yet implemented')
def test_jdbc_multitable_consumer_origin_configuration_transaction_isolation(sdc_builder, sdc_executor, configuration_value):
    pass


@pytest.mark.parametrize('configuration_value', [False, True])
@pytest.mark.skip('Not yet implemented')
def test_jdbc_multitable_consumer_origin_configuration_use_credentials(sdc_builder, sdc_executor, configuration_value):
    pass

...
def test_jdbc_multitable_consumer_origin_configuration_convert_timestamp_to_string(sdc_builder, sdc_executor,
                                                                                   ...
                                                                                   database):
    """Test `Convert Timestamp To String` configuration for JDBC Multi Table Consumer.
    When there is DATE column in database, normally SDC field of type DATE is created. When this option is enabled,
    field of type STRING got created.
    """
    src_table_prefix = get_random_string(string.ascii_lowercase, 6)
    table_name = '{}_{}'.format(src_table_prefix, get_random_string(string.ascii_lowercase, 20))
    rows_in_database = [{'id': 1, 'name': 'Manish', 'last_updated': '2019-06-28 07:00:00'},
                        {'id': 2, 'name': 'Shravan', 'last_updated': '2019-06-28 07:00:00'},
                        {'id': 3, 'name': 'Shubham', 'last_updated': '2019-06-28 07:00:00'}]
    ...
    try:
        ...
        attributes = {'table_configs': [{"tablePattern": f'%{src_table_prefix}%'}],
                      'convert_timestamp_to_string': convert_timestamp_to_string}
        ...
        rows_from_snapshot = [tuples_to_lower_name(list(record.field.items())[2])
                              for record in snapshot_content(snapshot, jdbc_multitable_consumer)]
        if convert_timestamp_to_string:
            assert rows_from_snapshot == [('last_updated', '{}.0'.format(row['last_updated']))
                                          for row in rows_in_database]
        else:
            assert rows_from_snapshot == [('last_updated', str(row['last_updated'])) for row in rows_in_database]
    finally:

...
@database
@pytest.mark.parametrize('configure_environment', [False]) # True,
def test_jdbc_multitable_consumer_origin_configuration_connection_timeout_in_seconds(sdc_builder, sdc_executor,
                                                                                     database, configure_environment):
    """Check if JDBC multitable consumer honours Connection Timeout In Seconds configuration.
    We will check by providing valid connection string for which stage should connect to DB successfully.
    Then we will configure connection string with non-routable IP 10.255.255.1 so that connection to database
    server get timed out.
    Note : A non routable network is, a network that is not "visible" in the Internet.
    """
    ...
    try:
        ...
        attributes = {'table_configs': [{"tablePattern": f'%{src_table_prefix}%'}],
                      'connection_timeout_in_seconds': '${10 * SECONDS}'}
        ...
        if not configure_environment:
            connection_string = jdbc_multitable_consumer.jdbc_connection_string
            jdbc_multitable_consumer.jdbc_connection_string = re.sub('\/.*?\:', '//10.255.255.1:',
                                                                     connection_string, flags=re.DOTALL)
        ...
        if configure_environment:
            ...
            try:
                ...
                # JDBC_00 error code for not able to connect to database. There is no error code listed in
                # JDBCErrors.java for connection timeout.
                logger.info('JDBC Multitable Consumer execution throws exeception')
                assert 'JDBC_00' in e.message
            else:
                assert False
    finally:
...
def get_jdbc_multitable_consumer_to_trash_pipeline(sdc_builder, database, attributes, configure_environment = True):

...
@database
def test_mongodb_oplog_origin(sdc_builder, sdc_executor, database):
    """
    Insert data in MongoDB and then check if MongoDB Oplog origin captures changes in data or the database correctly.
    ...
    try:
        ...
        mongodb_database = database.engine[database_name]
        ...
        database.engine.drop_database(database_name)

@database
def test_mongodb_origin_simple(sdc_builder, sdc_executor, database):
    """
    ...
    try:
        ...
        mongodb_database = database.engine[mongodb_origin.database]
        ...
        database.engine.drop_database(mongodb_origin.database)

@database
def test_mongodb_destination(sdc_builder, sdc_executor, database):
    """
    ...
    # MongoDB destination uses the CRUD operation in the sdc.operation.type record header attribute when writing to MongoDB.
    # value 4 specified below is for UPSERT.
    expression_evaluator.header_expressions = [{'attributeToSet': 'sdc.operation.type', 'headerAttributeExpression': '4'}]
    ...
    try:
        ...
        assert [item['text'] for item in database.engine[mongodb_dest.database][mongodb_dest.collection].find()] == DATA

    finally:
        logger.info('Dropping %s database...', mongodb_dest.database)
        database.engine.drop_database(mongodb_dest.database)
